...
export class TextEditor extends React.Component {
    ...
    componentWillReceiveProps(nextProps) {
        ...
        if (nextProps.variableToAdd) {
            this.insertVariable(nextProps.variableToAdd, true);
        }

        if (nextProps.contentToReplace) {
            this.insertTemplateWOReplace(nextProps.contentToReplace);
            nextProps.onContentReplaced();
        }
    }
    ...
    insertVariable(variable, noReplace) {
        const editorState = this.state.editorState;
        const selection = editorState.getSelection();
        ...
        if (this.props.onVariableInserted) {
            this.props.onVariableInserted();
        }
    }
    ...
    insertTemplateWOReplace(tpl) {
        const tplContentState = stateFromHTML(tpl);
        const es = EditorState.push(this.state.editorState, tplContentState, 'replace-state');
        this.onChange(es);
    }
